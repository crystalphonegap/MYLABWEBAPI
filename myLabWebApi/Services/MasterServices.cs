...
namespace myLabWebApi.Services
{
    public class MasterServices : IMaster
    {
        ...
        public MasterServices(IMyLabHelper MyLabHelper, ILogger ILoggerservice, IConfiguration config)
        {
            _config = config;
        ...
        public List<CollectionCenterModel> GetAllCollectionCenterDetails()
        {
            var dbPara = new DynamicParameters();
            var data = _MyLabHelper.GetAll<CollectionCenterModel>("[dbo].[SP_GetCollectionCenterDetail]", dbPara, commandType: CommandType.StoredProcedure);
            return data.ToList();
        }
        public long InsertUpdateCollectioncenter(CollectionCenterModel orderHeaderdetails)
        {
            var dbPara = new DynamicParameters();
